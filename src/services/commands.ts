...
const commands: any = Normalize.commands({
    help: {
        command: help,
...
export async function help(message: any) {
    await message.reply(dedent`
        Here's a list of the available commands!

        ${Object.entries(commands).map(([name, { description, args }]: any) => dedent`
            \`${Concat.concat(
                `.${name}`,
...
export async function roll(message: any, size: number) {
    await message.reply(`You rolled a ${Random.integer(1, size)}.`);
}

export async function rolld4(message: any) {
    await roll(message, 4);
}

export async function rolld6(message: any) {
    await roll(message, 6);
}

export async function rolld8(message: any) {
    await roll(message, 6);
}

export async function rolld12(message: any) {
    await roll(message, 12);
}

export async function rolld20(message: any) {
...
export async function rolld100(message: any) {
...
export function ProcessCommand(message: any) {
